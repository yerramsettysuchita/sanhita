...
5. [What is actually in here](#what-is-actually-in-here)
6. [Stage one, reading the PDF](#stage-one-reading-the-pdf)
7. [Stage two, the clause tree](#stage-two-the-clause-tree)
8. [Stage three, the Obligation IR](#stage-three-the-obligation-ir)
9. [Stage four, extraction](#stage-four-extraction)
10. [Stage five, human certification](#stage-five-human-certification)
11. [Stage six, deterministic execution](#stage-six-deterministic-execution)
12. [Stage seven, the remediation loop](#stage-seven-the-remediation-loop)
13. [Operational mapping](#operational-mapping)
14. [Amendments](#amendments)
15. [Cross rule analysis](#cross-rule-analysis)
16. [Coverage and its denominator](#coverage-and-its-denominator)
17. [Determinism](#determinism)
18. [Provenance](#provenance)
19. [Performance](#performance)
20. [Every screen](#every-screen)
21. [Testing](#testing)
...
The image carries the circular and the compiled rulebook, so the container is
self-sufficient and makes no network call at any point.
...
| Time to compile the whole circular | **about 1 second** |
...
```mermaid
flowchart TD
    S["40 Â· SECTION"] --> C1["40.1 Â· CLAUSE"]
    S --> C2["40.2 Â· CLAUSE"]
    C1 --> SC1["40.1.1 Â· SUBCLAUSE"]
    C1 --> SC2["40.1.8 Â· SUBCLAUSE"]
    SC2 --> I1["(i) Â· ITEM"]
    SC2 --> I2["(ii) Â· ITEM"]
```
...
Obligation(
    ...
    action=Action(verb="issue", object="a daily margin statement",
                  recipient="every client"),
    trigger=Trigger(kind=TriggerKind.SCHEDULE, expression="each trading day",
                    recurrence="FREQ=DAILY"),
    deadline=Deadline(kind=DeadlineKind.END_OF_PERIOD, period="DAY"),
    conditions=[Condition(kind=ConditionKind.THRESHOLD, expression="...")],
    evidence=[EvidenceReq(artifact_type="dispatch log"),
              EvidenceReq(artifact_type="client acknowledgement")],
    source=SourceAnchor(clause_id="40.1.8", page=95, char_span=(412_88, 413_51),
                        verbatim_text="...", sha256="4f2a...9c"),
    confidence=0.81,
    status=RuleStatus.CERTIFIED,
    certification=Certification(certified_by="...", signature="..."),
)
```
...
931 conditions across the corpus, of which about 5% contain so much as a
...
|---|---|---|
| Whole circular | **about 1 second** | roughly 40 hours |
...
`sanhita audit --verify` walks the chain. If an entry has been altered, the
chain stops verifying and the screen says which entry and why.
...
**30 findings**, 15 not applicable and 138 undetermined. Before this, zero.
...
```mermaid
flowchart LR
    CL[clause 114.2] --> OB[obligation]
    OB --> PR[Daily margin reporting]
    PR --> FN[Operations]
    FN --> SY[Margin engine]
    SY --> CT[SOP-12]
...
```
Draft amendment to clause 27.4, deadline T+5 becomes T+1

  Rules directly changed                    1
  Rules requiring re-read (transitive)      7
  New contradictions introduced             2   <- clause 31.2 still says T+5
  Calendar occasions added per year       812
  Actors affected      stock broker, compliance officer
```
...
| Signal | What it is |
|---|---|
| `ambiguity` | extractor confidence in the lower quartile for this document |
| `unresolved` | fields the extractor refused to fill |
| `judgement` | conditions in prose with nothing to measure |
| `contested` | a reviewer already overruled the machine here |

**18 clauses** carry two or more independent signals. Redraft those eighteen
and the confusion never happens. This is a ranking, not a probability, and the
screen says so.
...
signed over, taken with 1.27.2. On a different version those hashes stop
...
|---|---:|---|
| PDF to layout, cold | 1.06 s | 399 pages, 765,120 characters |
| Layout to clause tree, cold | 1.13 s | 2,717 clauses |
| Fingerprint the tree, warm | 1.6 ms | hashes every id, span and text |
| **Extract obligations** | **908 ms** | 1,377 obligations, about 1,500 a second |
| Load the signed store | 66 ms | 1,377 rules, 1,560 ledger entries |
| Verify the audit chain | 13 ms | 1,560 hash chained entries |
| Compute coverage | 145 ms | classifies 1,720 clauses |
| Run every certified rule | 2.1 ms | 279 occasions, 71 findings |
| Find contradictions | 6.6 ms | 141 pairs compared in full |
| Rank divergence risk | 6.0 ms | 807 clauses scored |
| Measure regulatory load | 1.4 ms | 6 actors, 1,377 rules |
...
| | |
|---|---:|
| Container cold start to first response | 3.4 s |
| Steady memory | 154 MB |
| Image size | 650 MB |
...
**Conditions hold prose, not predicates.** 931 of them, about 5% containing a
...
Seven master circulars and four ordinary ones, 11,262 clauses. Two real defects
surfaced doing that and both are fixed.
...
```
src/sanhita/
â”œâ”€â”€ parse/           PDF to clause tree
â”‚   â”œâ”€â”€ layout.py        lines, footnotes, page furniture, measured body size
â”‚   â”œâ”€â”€ clause_tree.py   numbering, depth, flat fallback, spans and hashes
â”‚   â”œâ”€â”€ footnotes.py     lineage, 133 of 134 resolved
â”‚   â””â”€â”€ quality.py       Readable, With care, or No
â”œâ”€â”€ ir/              the typed artifact
â”‚   â”œâ”€â”€ schema.py        Obligation and every value object
â”‚   â”œâ”€â”€ canonical.py     byte stable JSON, the signing payload
â”‚   â””â”€â”€ enums.py         Actor, Modality, DeadlineKind, DayCount
â”œâ”€â”€ compile/         extraction
â”‚   â”œâ”€â”€ extract.py       deterministic, no network
â”‚   â”œâ”€â”€ llm.py           model assisted, span verified
â”‚   â””â”€â”€ temporal.py      deadlines, periods, day counts
â”œâ”€â”€ certify/         the human boundary
â”‚   â”œâ”€â”€ lifecycle.py     versions, status, point in time replay
â”‚   â””â”€â”€ ledger.py        append only, hash chained
â”œâ”€â”€ execute/         deterministic run
â”‚   â”œâ”€â”€ applicability.py was anything owed, before evidence is read
â”‚   â”œâ”€â”€ engine.py        due dates, outcomes, refusals
â”‚   â”œâ”€â”€ evidence.py      the firm's records
â”‚   â””â”€â”€ report.py        findings with citations
â”œâ”€â”€ remediate/       closing the gap
â”‚   â”œâ”€â”€ tasks.py         the task, its lifecycle, its log
â”‚   â””â”€â”€ service.py       where a task meets the engine
â”œâ”€â”€ diff/            amendments
â”œâ”€â”€ analyse/         conflicts, divergence, burden, forecast, impact
â”œâ”€â”€ metrics/         coverage and its denominator
â”œâ”€â”€ controls.py      process, team, system, procedure
â”œâ”€â”€ benchmark.py     every performance figure in this file
â””â”€â”€ web/             FastAPI, Jinja, vanilla JS

corpus/              SEBI circulars, the stock broker one committed
.sanhita/            compiled store, signatures, ledgers
eval/                gold set and harness
...
Python 3.12, Pydantic v2, PyMuPDF pinned to 1.27.2.2, Typer, FastAPI, Jinja2,
uvicorn, pytest.
